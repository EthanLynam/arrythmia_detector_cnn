...
# IT IS FOR THE CNN TO USE LATER TO DIVIDE THEM INTO TRAINING, VAL, OR TESTING DATA.
    ...
    dt_name = annotation_map.get(beat_label, 'OTHER')
    ...
    for data, name in pairs:
        ...
        fig.savefig(
            f'data/created_images/{dt_name}/{patient_num}_{idx}_{name}.png',
            bbox_inches='tight',
            dpi=100,
            pad_inches=0
            )
    ...
    print(f"******* Heartbeat {idx}: ARRYTHMIA - AUGMENTED IMAGES CREATED *******")
